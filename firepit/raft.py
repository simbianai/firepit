    ...
    for k, v in d.items():
        if "-" in k:  # Weird STIX rule: single quotes around things like SHA-1
            if ":" in k:
                ...
                parts = path.split(".")
                key = f"{otype}:" + ".".join(
                    [f"'{part}'" if "-" in part else part for part in parts]
                )
            else:
                key = f"'{k}'"
        else:
        ...
        if k == "extensions" or (
            isinstance(v, dict)
            and not (isinstance(otype, str) and otype.startswith("x-"))
        ):
...
def flatten_21(obj):
    ...
    if obj_type == "identity":
        ...
        for ref in obj["object_refs"]:
            ...
            results.append(
                {"type": "__contains", "source_ref": oid, "target_ref": str(ref)}
            )
        del obj["object_refs"]
...
def flatten(obs):
    ...
    for idx, orig_sco in scos.items():
        ...
        results.append(
            {"type": "__contains", "source_ref": obs["id"], "target_ref": sco["id"]}
        )
